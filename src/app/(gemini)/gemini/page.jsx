...
export default function AgentHome() {
  ...
  const [displayText, setDisplayText] = useState('Tap anywhere to speak...');
  ...
  useEffect(() => {
      ...
      if (spokenText.includes(agent.name.toLowerCase())) {
        ...
        try {
          ...
          const matchedVoice = voices.find((v) =>
            v.name.toLowerCase().includes(agent.name.toLowerCase())
          );
          ...
          setDisplayText('❌ Error fetching assistant response');
        }
      } else {
    ...
    recognition.onerror = (event) => {
      setDisplayText(`Speech error: ${event.error}`);
    };
